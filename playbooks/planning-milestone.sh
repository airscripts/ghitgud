...
MILESTONE_TITLE="gitfleet-test-ms"
...
teardown() {
  if [ "$MILESTONE_CREATED" = true ] && [ -n "$MILESTONE_NUMBER" ]; then
    ...
    gitfleet planning milestone close "$MILESTONE_TITLE" --repo "$REPO" >/dev/null 2>&1 && \
...
output=$(gitfleet planning milestone create --title "$MILESTONE_TITLE" --due "2099-12-31" --repo "$REPO" --json 2>&1) || true
...
if [ "$MILESTONE_CREATED" = true ]; then
  ...
  expect_exit_0 "milestone progress succeeds" gitfleet planning milestone progress "$MILESTONE_TITLE" --repo "$REPO"
else
...
if [ "$MILESTONE_CREATED" = true ]; then
  ...
  expect_exit_0 "milestone close succeeds" gitfleet planning milestone close "$MILESTONE_TITLE" --repo "$REPO"
  MILESTONE_CREATED=false
else
...
expect_exit_non0 "milestone create without title fails" gitfleet planning milestone create --repo "$REPO"
