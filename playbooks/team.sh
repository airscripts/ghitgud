...
setup() {
  if ! gh api "orgs/$ORG/teams/$TEST_TEAM" >/dev/null 2>&1; then
    ...
    if ghg team create --org "$ORG" --name "$TEST_TEAM" --description "ghg playbook test team" >/dev/null 2>&1; then
...
teardown() {
  if [ "$MEMBER_ADDED" = true ]; then
    ...
    ghg team remove --org "$ORG" --team "$TEST_TEAM" --user "$TEST_USER" >/dev/null 2>&1 && \
...
step "List Teams"
expect_exit_0 "team list succeeds" ghg team list --org "$ORG"

step "List Teams JSON"
expect_json_field "JSON has success=true" "success" "true" ghg team list --org "$ORG"

step "Add Team Member"
if ghg team add --org "$ORG" --team "$TEST_TEAM" --user "$TEST_USER" >/dev/null 2>&1; then
...
if [ "$MEMBER_ADDED" = true ]; then
  ...
  if ghg team remove --org "$ORG" --team "$TEST_TEAM" --user "$TEST_USER" >/dev/null 2>&1; then
...
CI=true expect_exit_non0 "team create without --name fails" ghg team create --org "$ORG"

step "Team Add Without --team"
CI=true expect_exit_non0 "team add without --team fails" ghg team add --org "$ORG" --user "$TEST_USER"
