...
teardown() {
  if [ "$VAR_SET" = true ]; then
    ...
    gitfleet variable delete --name "$VAR_KEY" --repo "$REPO" >/dev/null 2>&1 && \
...
step "Set A Variable"
if gitfleet variable set --name "$VAR_KEY" --value "$VAR_VALUE" --repo "$REPO" >/dev/null 2>&1; then
...
if [ "$VAR_SET" = true ]; then
  ...
  expect_exit_0 "variable delete succeeds" gitfleet variable delete --name "$VAR_KEY" --repo "$REPO"
  VAR_SET=false
else
...
expect_exit_non0 "variable set without name fails" gitfleet variable set --value test --repo "$REPO"
