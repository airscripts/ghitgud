...
const ERROR_MAP: Record<number, typeof GitfleetError> = {
...
function handleRateLimit(response: Response): never {
  ...
  if (!rateLimit) {
    throw new GitfleetError(ERROR_UNEXPECTED);
  }
...
function handleError(
  ...
  if (status === STATUS_RATE_LIMITED) {
    ...
    throw new GitfleetError("Rate limit exceeded.");
  }

  const ErrorClass = ERROR_MAP[status];
  if (ErrorClass) throw new ErrorClass(ERROR_MESSAGES[status]);
  throw new GitfleetError(`${ERROR_UNEXPECTED}: ${status}`);
}
...
function getApiBaseUrl(host = config.read("host") ?? "github.com"): string {
  return host === "github.com" ? GITHUB_API_BASE_URL : `https://${host}/api/v3`;
}

function getGraphqlUrl(host = config.read("host") ?? "github.com"): string {
...
async function getPaginated<T>(endpoint: string): Promise<T[]> {
  let nextUrl: string | null = `${getApiBaseUrl()}${endpoint}`;
  const results: T[] = [];
...
const client = {
  ...
  validateToken: (token: string, host = "github.com") =>
    requestUrl(`${getApiBaseUrl(host)}/user`, {}, token),
