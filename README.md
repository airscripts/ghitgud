...
[![License](https://img.shields.io/github/license/airscripts/gitfleet)](https://github.com/airscripts/gitfleet/blob/main/LICENSE)
[![Coverage](https://img.shields.io/badge/coverage-87%25-brightgreen)](./coverage)

Command every repository as one fleet.

Gitfleet is a human-first CLI and full-screen TUI for repository management
across providers. GitHub is the first provider; GitLab is planned against the
...
| -------------------- | ---------------------------------------------------- |
| `domain`             | Provider-neutral references and capabilities         |
| `application`        | Rendering-free workflows and bulk execution          |
| `operations`         | Canonical command families and surface registration  |
| `providers/github`   | GitHub client, endpoints, and wire integration       |
| `commands` and `tui` | Thin CLI and interactive surfaces                    |
| `core`               | Config, Git, filesystem, output, prompts, and errors |
| `api` and `services` | Compatibility layers being migrated inward           |

Only provider clients perform HTTP requests. Unsupported operations return a
capability error instead of pretending another provider supports them.
...
- Named profiles, explicit JSON output, terminal themes, aliases, completion,
  reversible live playbooks, and a full-screen TUI.

## Installation

Gitfleet requires Node.js 24 or newer and pnpm 10 for development.

```bash
npm install --global gitfleet
gitfleet version
gf version
```
...
For CI, set `GITFLEET_GITHUB_TOKEN`. Destructive operations in JSON or
...
Credentials are stored at `~/.config/gitfleet/credentials.json` with mode
...
| `alias`, `completion`, `config`, `tui`, `help` | Gitfleet utilities                                         |
...
- `change queue`, `planning milestone`, `policy branch`, and `repo forks`
...
The TUI consumes the same canonical command registry and hides operations that
...
Live playbooks under `playbooks/` validate command families against the GitHub
API. They require explicit credentials and revert mutations during teardown.

```bash
REPO=owner/test-repository ORG=example bash playbooks/all.sh
SKIP="pipeline-run.sh,planning.sh" bash playbooks/all.sh
...
functions, and lines. See [AGENTS.md](./AGENTS.md) for architecture, testing,
style, playbook, and release requirements.

## Repository Structure

```text
src/
  application/       rendering-free workflows and bulk orchestration
  domain/            provider-neutral contracts, references, and errors
  providers/github/  GitHub provider, HTTP client, and endpoint adapters
  operations/        canonical families and CLI registration
  commands/          thin Commander command modules
  tui/               full-screen interface and operation catalog
  core/              config, Git, output, prompts, filesystem, and errors
  api/               temporary provider compatibility re-exports
  services/          workflows being migrated into application use cases
tests/
  unit/              domain, provider, operation, command, service, and TUI tests
  integration/       command integration tests
playbooks/            reversible live GitHub validation
templates/            built-in label templates
```
...
For usage questions, run `gitfleet help [command...]`. For reproducible bugs,
open an issue with the Gitfleet version, operating system, command, expected
behavior, actual behavior, and redacted debug output.
