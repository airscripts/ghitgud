...
describe("pr command", () => {
  ...
  it("registers pr command with subcommands", () => {
    ...
    const pr = program.commands.find((command) => command.name() === "change");
  ...
  it("rejects invalid PR numbers before calling service", async () => {
    ...
    await expect(
      program.parseAsync(["node", "test", "change", "push", "42abc"]),
  ...
  it("close subcommand has --comment option", () => {
    ...
    const pr = program.commands.find((c) => c.name() === "change");
