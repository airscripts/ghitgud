...
import licensesCommand from "@/commands/license";
...
describe("licenses command", () => {
  ...
  it("should register licenses command with subcommands", () => {
    ...
    const licenses = program.commands.find((c) => c.name() === "license");
  ...
  it("should call list service on licenses list", async () => {
    ...
    await program.parseAsync(["node", "test", "license", "list"]);
  ...
  it("should call view service on licenses view with key", async () => {
    ...
    await program.parseAsync(["node", "test", "license", "view", "mit"]);
